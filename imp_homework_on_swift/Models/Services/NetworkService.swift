...
class NetworkService : PlanetListNetworkService {
    ...
    func getPlanetLits(page: Int, onRequestComleted: @escaping ((PlanetListRespondsModel?, Error?) -> ())) {
        performGetRequest(urlString: NetworkConstants.URLString.planetList + "?page=\(page)", onRequestComleted: onRequestComleted)
    }

    private func performGetRequest<ResponseModel: Decodable>(urlString: String, method: HTTPMethod = .get, onRequestComleted: @escaping ((ResponseModel?, Error?)->())) {
